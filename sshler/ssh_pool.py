...
class SSHConnectionPool:
    ...
    async def acquire(
        ...
        if last_fail and (time.time() - last_fail) < self._fail_cooldown:
            raise ConnectionError(f"{box_name}: unreachable (cached, retry in {int(self._fail_cooldown - (time.time() - last_fail))}s)")
        ...
        async with self._locks[box_name]:
            ...
            try:
                ...
                self._fail_cache[box_name] = time.time()
                raise
