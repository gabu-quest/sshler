...
const buildBoxQuery = () => {
  ...
  const dir = appStore.activeDir;
  return box
    ? `?box=${encodeURIComponent(box)}${dir ? `&dir=${encodeURIComponent(dir)}` : ''}`
    : '';
};
...
const links = computed(() => {
  ...
  return [
    ...
    { to: `/commander${boxQuery}`, label: t("commander.title"), icon: PhColumns, shortcut: "Alt+C" },
