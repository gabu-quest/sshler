...
import { useMessage, NTooltip, NPopconfirm, NModal, NPopover } from 'naive-ui'
import {
  PhCaretLeft,
  PhCaretRight,
  PhPlus,
...
interface Emits {
  ...
  (e: 'session-select', session: ApiSession): void
}
...
// A native Windows shell (pwsh/powershell/cmd/wsl) is plain ConPTY with no tmux.
...
// In-place session switch: overrides prop values for next connect() call only
const sessionOverride = ref<string | null>(null)
const directoryOverride = ref<string | null>(null)
// The session this terminal was originally spawned with — used as "home" in the switcher
const spawnSession = ref<string>(props.sessionName ?? 'main')
// Deterministic per-session color, matching the SessionSwitcher accent.
const sessionColor = computed(() => getColorForSession(sessionOverride.value ?? props.sessionName ?? 'main'))
...
let snapshotVisHandler: (() => void) | null = null
...
// RAF-batched write queue — coalesces incoming WS frames into one paint per frame.
// Keeps the tab responsive when scrollback is large or output is heavy (e.g. log tails).
const writeQueue: Uint8Array[] = []
let writeQueuedBytes = 0
let writeRafId: number | null = null
// Hard cap: if a backgrounded tab accumulates more than this, drop oldest chunks.
const WRITE_QUEUE_MAX_BYTES = 4 * 1024 * 1024 // 4 MiB

const flushWriteQueue = () => {
  writeRafId = null
  if (!terminal || writeQueue.length === 0) return
  // Don't paint while hidden — frames are throttled anyway and writes accumulate
  // visible work that's wasted. Stay queued; visibilitychange will re-schedule.
  if (document.hidden) return
  const chunks = writeQueue.splice(0, writeQueue.length)
  writeQueuedBytes = 0
  // Merge all pending chunks into one buffer so the renderer sees a single
  // write per frame instead of N — reduces glyph-placement state transitions
  // that cause visible character corruption under heavy output (e.g. Claude Code).
...
const scheduleFlush = () => {
  ...
  if (document.hidden) return
  writeRafId = requestAnimationFrame(flushWriteQueue)
}

const queueWrite = (data: Uint8Array | string) => {
  const chunk = typeof data === 'string' ? textEncoder.encode(data) : data
  writeQueue.push(chunk)
  writeQueuedBytes += chunk.byteLength
  // Backpressure: drop oldest if a hidden tab accumulates too much. Visible
  // tabs drain per-frame, so this only fires for long-backgrounded panes.
  while (writeQueuedBytes > WRITE_QUEUE_MAX_BYTES && writeQueue.length > 1) {
    const dropped = writeQueue.shift()
    if (dropped) writeQueuedBytes -= dropped.byteLength
  }
  scheduleFlush()
}
...
// Live working directory, learned from OSC escapes the shell emits on each
// prompt (OSC 7 file://, ConEmu/Windows-Terminal OSC 9;9, iTerm2 OSC 1337
// CurrentDir). Null until the shell reports one — we then prefer it over the
// terminal's *initial* directory so clicked relative paths resolve against
// where the user actually `cd`'d to, not where the tab opened.
const currentCwd = ref<string | null>(null)

/** True for POSIX (`/x`), Windows drive (`C:\x` / `C:/x`), or UNC (`\\srv`) absolutes. */
const isAbsolutePath = (p: string): boolean =>
  p.startsWith('/') || /^[A-Za-z]:[\\/]/.test(p) || p.startsWith('\\\\')

/** Collapse `.`/`..` segments, preserving a `C:` drive or `//` UNC root. Input/output
 *  use forward slashes — the backend's local-Windows file API speaks POSIX (`C:/Users/x`). */
const normalizeAbsolute = (absPath: string): string => {
  const p = absPath.replace(/\\/g, '/')
  const drive = p.match(/^([A-Za-z]:)\//)?.[1] ?? ''
  const isUnc = !drive && p.startsWith('//')
  ...
  if (drive) return out.length ? `${drive}/${out.join('/')}` : `${drive}/`
  if (isUnc) return `//${out.join('/')}`
  return `/${out.join('/')}`
}

/** Resolve a (possibly relative) path against a base directory. Handles both POSIX
 *  and Windows-shaped bases, and never force-prepends `/` to a `C:`-rooted path. */
const resolvePathAgainst = (rawPath: string, base: string): string => {
  if (isAbsolutePath(rawPath)) return normalizeAbsolute(rawPath)
  const b = base.replace(/\\/g, '/')
  const drive = b.match(/^([A-Za-z]:)\//)?.[1] ?? ''
  const baseBody = drive ? b.slice(drive.length) : b
  const parts = baseBody.split('/').filter(Boolean)
  for (const seg of rawPath.replace(/\\/g, '/').split('/')) {
    if (seg === '.' || seg === '') continue
...
/** Extract a directory from an OSC cwd payload (OSC 7 URI or a bare path) and
 *  normalize it to the backend's POSIX form (`C:/Users/x`, no leading slash on a drive). */
const parseOscCwd = (input: string): string | null => {
  let p = (input || '').trim()
  if (!p) return null
  if (p.startsWith('file://')) {
    try {
      // pathname is `/C:/Users/x` on Windows, `/home/x` on POSIX
  ...
  p = p.replace(/\\/g, '/')
  // Windows file:// pathnames arrive as `/C:/...` — drop the leading slash before a drive.
  if (/^\/[A-Za-z]:\//.test(p)) p = p.slice(1)
  if (p.length > 1) p = p.replace(/\/+$/, '')
  return p || null
}

const updateCwdFromOsc = (payload: string) => {
  const cwd = parseOscCwd(payload)
...
const handleFilePathClick = async (rawPath: string) => {
  ...
  // Open the tab synchronously within the user gesture so popup blockers don't fire.
  // window.open called after an await loses the gesture context and gets blocked.
  const win = window.open('about:blank', '_blank')

  // Home-relative paths (`~/…`) are passed through untouched — the backend expands `~`.
  // Everything else resolves against the live cwd (from OSC) when known, else the
  // terminal's initial directory, normalized to the POSIX form the local-Windows
  // file API expects (Windows-aware: drive letters, UNC, `..` collapse).
  let resolved = rawPath
  if (!rawPath.startsWith('~/') && rawPath !== '~') {
    const base = currentCwd.value || props.directory || '/'
    resolved = resolvePathAgainst(rawPath, base)
  }
  ...
  try {
    ...
    const url = `/app/files?${params.toString()}`
    if (win) {
...
const scrollbackText = ref('')
const scrollbackTextareaRef = ref<HTMLTextAreaElement | null>(null)
const scrollbackSource = ref<'xterm' | 'tmux'>('xterm')
const scrollbackLoading = ref(false)

// Pull the full xterm scrollback + viewport as plain text.
// Walks buffer.active from line 0 to baseY+rows. Trims trailing blank lines so
// the textarea doesn't start with a wall of empty lines for short sessions.
const getScrollbackText = (): string => {
  if (!terminal) return ''
...
const copyAllScrollback = async () => {
  const text = getScrollbackText()
  if (!text) {
    message.warning(t('terminal.scrollback_empty'))
    return
  }
  await writeClipboard(text)
  const lineCount = text.split('\n').length
  message.success(t('terminal.scrollback_copied', { lines: lineCount, chars: text.length }), { duration: 1500 })
}

const focusAndScrollToBottom = async () => {
  await nextTick()
  const ta = scrollbackTextareaRef.value
  if (ta) {
    ta.focus()
...
const openScrollbackViewer = async () => {
  scrollbackSource.value = 'xterm'
  scrollbackText.value = getScrollbackText()
  showScrollback.value = true
  await focusAndScrollToBottom()
}

const loadTmuxHistory = async () => {
  if (!props.sessionName) {
    message.warning(t('terminal.scrollback_no_session'))
    return
  }
  scrollbackLoading.value = true
  try {
    ...
    scrollbackText.value = result.text
    scrollbackSource.value = 'tmux'
    message.success(t('terminal.scrollback_loaded_tmux', { lines: result.lines, chars: result.chars }), { duration: 1500 })
    await focusAndScrollToBottom()
  } catch (err: any) {
    console.warn('[Terminal] tmux capture-pane failed:', err)
    message.error(t('terminal.scrollback_tmux_failed', { err: err?.message ?? String(err) }))
  } finally {
    scrollbackLoading.value = false
  }
}

const reloadXtermScrollback = async () => {
  scrollbackText.value = getScrollbackText()
  scrollbackSource.value = 'xterm'
  await focusAndScrollToBottom()
}

const selectAllScrollbackText = () => {
  scrollbackTextareaRef.value?.select()
}

const copyCurrentScrollbackText = async () => {
  const text = scrollbackText.value
  if (!text) {
    message.warning(t('terminal.scrollback_empty'))
    return
  }
...
const sendTmuxCommand = (cmd: string) => {
  if (isNativeShell.value) return  // no tmux on native Windows shells
...
const loadCanvasRenderer = () => {
  ...
  try {
    ...
    console.warn('[Terminal] Canvas renderer failed; using DOM:', err)
    canvasAddon = null
  }
}

const loadAcceleratedRenderer = () => {
  if (!terminal) return
  // Try WebGL first.
  try {
...
const createTerminal = () => {
  ...
  // paths resolve correctly after a `cd`. Best-effort: shells that don't emit any
  // of these fall back to the terminal's initial directory.
  //   OSC 7  ;  file://host/path                 (bash/zsh/fish, Starship, etc.)
  //   OSC 9  ; 9 ; path                           (ConEmu / Windows Terminal / oh-my-posh)
  //   OSC 1337 ; CurrentDir=path                  (iTerm2 integration)
  terminal.parser.registerOscHandler(7, (data: string) => {
    updateCwdFromOsc(data)
    return true
  })
  terminal.parser.registerOscHandler(9, (data: string) => {
    if (data.startsWith('9;')) {
      updateCwdFromOsc(data.slice(2))
      return true
    }
  ...
  // Renderer: WebGL is 5–10× faster than DOM for heavy output and large scrollback.
  // Fall back to canvas (also GPU-accelerated, broader compat) on WebGL failure or context loss.
  ...
  terminal.attachCustomKeyEventHandler((event: KeyboardEvent) => {
    ...
    // Ctrl+Shift+A: open scrollback viewer (full buffer, native scroll+select)
    if (event.ctrlKey && event.shiftKey && (event.key === 'A' || event.key === 'a')) {
      event.preventDefault()
      openScrollbackViewer()
      return false
    }

    // Ctrl+Shift+C: copy entire scrollback to clipboard
...
const connect = async () => {
  ...
  try {
    ...
    const params = new URLSearchParams({
      host: props.boxName,
      dir: directoryOverride.value ?? props.directory ?? '~',
      session: sessionOverride.value ?? props.sessionName ?? 'main',
      cols: '120',
      rows: '32'
    })
    
    // Windows-only: pick which native shell the backend should spawn.
    if (props.shell) {
      params.set('shell', props.shell)
    }
    ...
    websocket.onmessage = async (event) => {
      ...
      if (event.data instanceof Blob) {
        ...
        queueWrite(new Uint8Array(buffer))
        triggerActivity()
        return
      }
    ...
    websocket.onclose = (event) => {
      ...
      if (event.code === 4403) {
        ...
        // WSL selected but no distro installed — toast, don't reconnect.
        message.error(t('terminal.wsl_not_installed'), {
          duration: 6000
        })
        reconnecting.value = false
...
onMounted(async () => {
  ...
  // Snapshot freshness polling — only when tab is visible
  ...
  const startSnapshotPolling = () => {
    if (snapshotPollInterval) return
    pollSnapshot()
    snapshotPollInterval = window.setInterval(pollSnapshot, 10000)
  }
  const stopSnapshotPolling = () => {
    if (snapshotPollInterval) { clearInterval(snapshotPollInterval); snapshotPollInterval = null }
  }
  snapshotVisHandler = () => {
    if (document.hidden) {
      stopSnapshotPolling()
      ...
      // Tab was backgrounded: xterm's renderer may have gone stale (rAF
      // throttling, container resized while hidden). Re-fit and force a
      // full redraw so the buffer paints again instead of showing black.
      nextTick(() => {
        try {
          fitAddon?.fit()
          if (terminal) terminal.refresh(0, terminal.rows - 1)
        } catch { /* best effort */ }
        // Drain whatever the WS queued while hidden.
        scheduleFlush()
      })
    }
  }
  document.addEventListener('visibilitychange', snapshotVisHandler)
  if (!document.hidden) startSnapshotPolling()
...
onUnmounted(() => {
  ...
  if (snapshotVisHandler) document.removeEventListener('visibilitychange', snapshotVisHandler)
  ...
  try { webglAddon?.dispose() } catch { /* noop */ }
  webglAddon = null
  try { canvasAddon?.dispose() } catch { /* noop */ }
  canvasAddon = null
...
const switchSession = (name: string, dir: string) => {
  sessionOverride.value = name
  directoryOverride.value = dir
  disconnect()
  nextTick(() => connect())
}
  ...
  <div class="terminal-wrapper" :class="{ 'has-activity': hasActivity, 'is-fullscreen': isFullscreen }">
    ...
    <div v-if="showTitleBar && !titleBarCollapsed" class="terminal-titlebar">
      <div class="titlebar-left">
        <!-- Yellow group: Windows (tmux-only) -->
        <div v-if="!isNativeShell" class="toolbar-group toolbar-yellow">
          <NTooltip :delay="400">
            <template #trigger>
        ...
        <!-- Purple group: Panes (tmux-only) -->
        <div v-if="!isNativeShell" class="toolbar-group toolbar-purple">
          <NTooltip :delay="400">
            <template #trigger>
        ...
        <!-- Red group: Destructive (tmux-only; native shells kill via SessionSwitcher) -->
        <div v-if="!isNativeShell" class="toolbar-group toolbar-red">
          <NPopconfirm
      ...
      <div class="titlebar-center">
        <span class="session-color-dot" :style="{ background: sessionColor }" />
        <span class="titlebar-text">{{ boxName }} — {{ sessionName }}</span>
      </div>
      <div class="titlebar-right">
        <NTooltip v-if="!isNativeShell" :delay="300">
          <template #trigger>
            <button
        ...
        <NTooltip v-if="!isNativeShell" :delay="400">
          <template #trigger>
        ...
        <NTooltip :delay="400">
          <template #trigger>
            <button class="titlebar-btn" @click="openScrollbackViewer">
              <PhListBullets :size="14" weight="duotone" />
            </button>
          </template>
          {{ t('terminal.scrollback_viewer_tip') }}
        </NTooltip>
        <NTooltip :delay="400">
          <template #trigger>
            <button class="titlebar-btn" @click="copyAllScrollback">
              <PhCopySimple :size="14" weight="duotone" />
            </button>
          </template>
          {{ t('terminal.copy_all_tip') }}
        </NTooltip>
        <NTooltip :delay="400">
          <template #trigger>
        ...
        <NPopover v-model:show="showSessionSwitcher" trigger="click" placement="bottom-end">
          <template #trigger>
            <NTooltip :delay="400">
              <template #trigger>
                <button class="titlebar-btn" :class="{ active: showSessionSwitcher }">
                  <PhList :size="14" weight="duotone" />
                </button>
              </template>
              {{ t('terminal.sessions') || 'Sessions' }}
            </NTooltip>
          </template>
          <SessionSwitcher
            :box-name="props.boxName"
            :token="tokenValue"
            :current-session="sessionOverride ?? props.sessionName"
            :home-session="spawnSession"
            @select="(s) => { emit('session-select', s); showSessionSwitcher = false }"
          />
        </NPopover>
        <NTooltip :delay="400">
          <template #trigger>
        ...
        <NTooltip :delay="200" :style="{ maxWidth: '320px' }">
          ...
          <div style="font-size: 12px; line-height: 1.5">
            ...
            <div><b>Ctrl+Shift+A</b> — {{ t('terminal.help_scrollback_viewer') }}</div>
            <div><b>Ctrl+Shift+C</b> — {{ t('terminal.help_copy_all') }}</div>
    ...
    <!-- Scrollback Viewer Modal: native textarea so OS scroll+select works perfectly -->
    <NModal
      :show="showScrollback"
      preset="card"
      :title="t('terminal.scrollback_viewer_title')"
      style="max-width: 95vw; width: 95vw; max-height: 92vh"
      @update:show="(v: boolean) => { showScrollback = v }"
    >
      <div class="scrollback-modal">
        <div class="scrollback-actions">
          <button
            class="scrollback-btn"
            :class="{ 'source-active': scrollbackSource === 'xterm' }"
            :disabled="scrollbackLoading"
            @click="reloadXtermScrollback"
          >
            <PhArrowsClockwise :size="14" weight="duotone" />
            {{ t('terminal.scrollback_source_xterm') }}
          </button>
          <button
            class="scrollback-btn"
            :class="{ 'source-active': scrollbackSource === 'tmux' }"
            :disabled="scrollbackLoading || !sessionName"
            @click="loadTmuxHistory"
          >
            <PhClockCounterClockwise :size="14" weight="duotone" />
            <span v-if="scrollbackLoading">{{ t('terminal.scrollback_loading') }}</span>
            <span v-else>{{ t('terminal.scrollback_source_tmux') }}</span>
          </button>
          <span class="scrollback-divider" />
          <button class="scrollback-btn" @click="selectAllScrollbackText">
            {{ t('terminal.select_all') }}
          </button>
          <button class="scrollback-btn primary" @click="copyCurrentScrollbackText">
            <PhCopySimple :size="14" weight="duotone" />
            {{ t('terminal.copy_all_btn') }}
          </button>
          <span class="scrollback-stats">
            {{ t('terminal.scrollback_stats', {
              lines: scrollbackText ? scrollbackText.split('\n').length : 0,
              chars: scrollbackText.length,
            }) }}
          </span>
          <button class="scrollback-btn close" @click="showScrollback = false">
            {{ t('common.close') }}
          </button>
        </div>
        <textarea
          ref="scrollbackTextareaRef"
          class="scrollback-textarea"
          readonly
          spellcheck="false"
          :value="scrollbackText"
          @keydown.stop
        />
        <div class="scrollback-hint">
          <span v-if="scrollbackSource === 'tmux'">{{ t('terminal.scrollback_hint_tmux') }}</span>
          <span v-else>{{ t('terminal.scrollback_hint_xterm') }}</span>
        </div>
      </div>
    </NModal>
  </div>
...
.session-color-dot {
  ...
  border-radius: 50%;
  flex-shrink: 0;
  box-shadow: 0 0 4px var(--session-dot-glow, rgba(0, 0, 0, 0.2));
}
...
/* =============================================================================
   SCROLLBACK VIEWER — native textarea so OS-level scroll + select + copy work
   ============================================================================= */
.scrollback-modal {
  display: flex;
  flex-direction: column;
...
.scrollback-btn:hover {
  background: var(--surface-hover, var(--surface-variant));
  border-color: var(--accent);
}

.scrollback-btn.primary {
  background: var(--accent);
  color: #fff;
  border-color: var(--accent);
}

.scrollback-btn.primary:hover {
  filter: brightness(1.08);
}

.scrollback-btn.close {
  margin-left: auto;
}

.scrollback-btn.source-active {
  border-color: var(--accent);
  color: var(--accent);
  background: var(--accent-bg, rgba(124, 93, 255, 0.12));
}

.scrollback-btn:disabled {
  opacity: 0.5;
  cursor: not-allowed;
}
...
.scrollback-textarea:focus {
  outline: none;
  border-color: var(--accent);
  box-shadow: 0 0 0 2px var(--accent-bg, rgba(124, 93, 255, 0.15));
}
