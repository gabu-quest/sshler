...
const PING_DEFAULT_DURATION_KEY = "sshler:ping:default-duration";
...
const readTerminalScrollback = (): number => {
  if (typeof localStorage === "undefined") return DEFAULT_TERMINAL_SCROLLBACK;
...
const readPingDefaultDuration = (): number | null => {
  if (typeof localStorage === "undefined") return null;
  const stored = localStorage.getItem(PING_DEFAULT_DURATION_KEY);
  if (stored) {
    const n = parseInt(stored, 10);
    if (n >= 1000 && n <= 300_000) return n;
  }
  return null;
};
...
export const useAppStore = defineStore("app", () => {
  ...
  const pingDefaultDuration = ref<number | null>(readPingDefaultDuration());
  ...
  const setPingDefaultDuration = (ms: number | null) => {
    pingDefaultDuration.value = ms;
    if (typeof localStorage !== "undefined") {
      if (ms === null) {
        localStorage.removeItem(PING_DEFAULT_DURATION_KEY);
      } else {
        localStorage.setItem(PING_DEFAULT_DURATION_KEY, ms.toString());
      }
    }
  };
  ...
  return {
    ...
    // Ping settings actions
    setPingDefaultDuration,
