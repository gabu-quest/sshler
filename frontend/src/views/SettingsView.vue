...
import { http, buildHeaders, fetchSnapshotConfig, updateSnapshotConfig } from '@/api/http'
...
const loadSnapshotConfig = async () => {
  snapshotLoading.value = true
  try {
    snapshotConfig.value = await fetchSnapshotConfig(tokenValue.value)
  } catch (error) {
...
const saveSnapshotConfig = async (config: { enabled?: boolean; interval?: number }) => {
  snapshotSaving.value = true
  try {
    snapshotConfig.value = await updateSnapshotConfig(config, tokenValue.value)
  } finally {
    snapshotSaving.value = false
  }
}

const handleSnapshotEnabledChange = async (enabled: boolean) => {
  const previous = snapshotConfig.value.enabled
  snapshotConfig.value.enabled = enabled
  try {
    await saveSnapshotConfig({ enabled })
  } catch (error) {
    snapshotConfig.value.enabled = previous
    message.error(error instanceof Error ? error.message : String(error))
  }
}

const handleSnapshotIntervalChange = async (interval: number | null) => {
  if (interval === null) return

  const previous = snapshotConfig.value.interval
  snapshotConfig.value.interval = interval
  try {
    await saveSnapshotConfig({ interval })
  } catch (error) {
    snapshotConfig.value.interval = previous
    message.error(error instanceof Error ? error.message : String(error))
  }
}
...
onMounted(async () => {
  ...
  await Promise.all([loadPoolConfig(), loadSnapshotConfig()])
})
</script>
...
<template>
  <div class="page">
      ...
      <NSpace vertical size="medium">
        ...
        <NAlert v-if="bootstrapStore.error" type="error" :title="t('settings.auth_error')">
          <div class="alert-content">
            <span>{{ bootstrapStore.error }}</span>
    ...
    <NCard :title="t('ping.setting.title')" size="medium">
      <NSpace vertical size="large">
        <div class="pool-setting">
          <label><strong>{{ t('ping.setting.auto_dismiss') }}</strong></label>
          <p class="setting-description">{{ t('ping.setting.auto_dismiss_help') }}</p>
          <NSpace align="center">
            <NSwitch
              :value="appStore.pingDefaultDuration !== null"
              @update:value="(v: boolean) => appStore.setPingDefaultDuration(v ? 5000 : null)"
              size="small"
            />
            <NInputNumber
              v-if="appStore.pingDefaultDuration !== null"
              :value="appStore.pingDefaultDuration"
              @update:value="(v: number | null) => v != null && appStore.setPingDefaultDuration(v)"
              :min="1000"
              :max="300000"
              :step="1000"
              style="width: 180px"
            >
              <template #suffix>{{ t('ping.setting.duration_suffix') }}</template>
            </NInputNumber>
          </NSpace>
        </div>
      </NSpace>
    ...
    <NCard :title="t('settings.snapshot')" size="medium">
      <NSpace vertical size="large">
        <div>
          <div class="pool-setting">
            <div class="pool-setting-header">
              <label><strong>{{ t('settings.snapshot_enabled') }}</strong></label>
              <NSwitch
                :value="snapshotConfig.enabled"
                @update:value="handleSnapshotEnabledChange"
          ...
          <div class="pool-setting">
            <label><strong>{{ t('settings.snapshot_interval') }}</strong></label>
            <NInputNumber
              :value="snapshotConfig.interval"
              @update:value="handleSnapshotIntervalChange"
              :min="5"
              :max="300"
              :step="5"
              style="width: 200px"
        ...
        <NAlert v-if="snapshotLoading" type="info">
          {{ t('common.loading') }}
        </NAlert>
      </NSpace>
    </NCard>
