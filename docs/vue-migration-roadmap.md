...
> **Status: COMPLETE (historical).** The Vue SPA reached parity and the legacy Jinja/HTMX UI — along with the temporary `--ui` toggle — has since been removed. `/` now redirects to `/app/` and the SPA is the only UI. This document is retained as a record of the migration.
